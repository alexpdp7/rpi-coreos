...
def cache_dir():
    ...
    p = (
        pathlib.Path(os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache")))
        / APP_NAME
    )
...
def coreos_installer_download(architecture, stream="stable"):
    ...
    out = coreos_installer(
        ...
        cache_dir(),
        "--insecure",
        subprocess_options={"capture_output": True},
    )
...
def coreos_installer(*args, subprocess_options=None, sudo=False):
    ...
    return subprocess.run(
        sudo_cmd + ("coreos-installer",) + args, check=True, **subprocess_options
    )
...
def get_efi_part(device):
    parts = json.loads(
        subprocess.run(
            ["lsblk", device, "-J", "-oLABEL,PATH"], check=True, capture_output=True
        ).stdout
    )
        ...
        try:
            subprocess.run(
                [
                    "sudo",
                    "cp",
                    root_dir / f"usr/share/uboot/rpi_{rpi_ver}/u-boot.bin",
                    tmpdir / f"rpi{rpi_ver}-u-boot.bin",
                ],
                check=True,
            )
            subprocess.run(
